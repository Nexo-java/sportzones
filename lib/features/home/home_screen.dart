...
///
/// Kegunaan:
/// - Shell utama aplikasi yang memuat halaman `HomePage`, `SearchPage`,
///   `SavePage`, dan `ProfilePage` pada bottom navigation.
/// - Mendengarkan koleksi `berita` dari Firestore untuk memperbarui daftar
///   berita secara realtime dan men-trigger notifikasi internal ketika ada
///   berita baru (setelah load awal).
/// - Menyimpan state animasi banner sukses saat menambah berita.
///
/// Catatan implementasi:
/// - Notifikasi internal hanya ditambahkan untuk item baru yang muncul
///   setelah flag _initialNewsLoaded bernilai true, sehingga tidak memicu
///   notifikasi saat pertama kali data dimuat.
...
class _HomeScreenState extends State<HomeScreen> with TickerProviderStateMixin {
  ...
  Animation<double> _entryFadeAnimation = const AlwaysStoppedAnimation<double>(
    1.0,
  );
  Animation<Offset> _entrySlideAnimation = const AlwaysStoppedAnimation<Offset>(
    Offset.zero,
  );
    ...
    _newsSubscription = _firestore
        ...
        .listen(
          (snapshot) {
            ...
            // Compute newly added news items (ignore on first load)
            if (_initialNewsLoaded) {
              final existingIds = _latestNews.map((e) => e.idBerita).toSet();
              final newlyAdded = remoteNews
                  .where((n) => !existingIds.contains(n.idBerita))
                  .toList(growable: false);
              for (final n in newlyAdded) {
                _addNotification(n);
              }
            }
  ...
  Future<void> _openAddNewsForm() async {
    ...
    setState(() {
      final existingIndex = _latestNews.indexWhere(
        (item) => item.idBerita == newNews.idBerita,
      );
      if (existingIndex >= 0) {
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      ...
      builder: (context, child) {
        final bookmarkCount =
            UserRepository.instance.getCurrentUser()?.savedNews.length ?? 0;
